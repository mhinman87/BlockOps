...
const MarkdownComponents = {
  ...
  h3: ({ children }) => {
    ...
    return <h3 className="text-lg font-bold text-gray-800 dark:text-gray-200 mt-6 mb-3">{children}</h3>;
  },
  h4: ({ children }) => (
  ...
  p: ({ children }) => {
    ...
    if (text.startsWith('Any neurological') || text.includes('until proven otherwise')) {
      return (
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/40 rounded-lg px-5 py-4 my-4">
          <p className="text-sm font-semibold text-red-800 dark:text-red-300 leading-relaxed">{children}</p>
        </div>
      );
    }
    // Detect notes
    if (text.startsWith('Note:') || text.startsWith('*Note')) {
      return (
        <div className="bg-gray-50 dark:bg-dark-bg border-l-3 border-gray-300 dark:border-dark-border rounded-r px-4 py-2 my-3">
          <p className="text-sm text-gray-600 dark:text-gray-300 font-light leading-relaxed italic">{children}</p>
        </div>
      );
    }
    return <p className="text-sm text-gray-700 dark:text-gray-300 font-light leading-relaxed mb-3">{children}</p>;
  },
  blockquote: ({ children }) => (
  ...
  td: ({ children }) => {
    ...
    if (/^\*\*/.test(text) || /^\d/.test(text)) {
      return <td className="px-4 py-2.5 text-sm font-semibold text-gray-900 dark:text-gray-100">{children}</td>;
    }
    return <td className="px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 font-light">{children}</td>;
  },
  ul: ({ children }) => (
    ...
    if (text.startsWith('☐ ') || text.startsWith('☑ ')) {
      ...
      return (
        <li className="flex items-start gap-3 py-1.5 px-3 bg-gray-50 rounded-lg">
          ...
          <span className="text-sm text-gray-700 dark:text-gray-300 font-light leading-relaxed">{text.slice(2)}</span>
        </li>
      );
    }
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <div className="bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-xl overflow-hidden mb-6">
          {/* Top bar */}
          ...
          <div className="px-6 py-5">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">{deliverable.title}</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 font-light mt-1">{deliverable.description}</p>
          ...
          {deliverable.storagePath && userRole?.isReviewer && currentStatus !== 'approved' && (
            <div className="px-6 py-3 bg-gray-50 dark:bg-dark-bg border-t border-gray-200 dark:border-dark-border flex gap-3">
              ...
              <button
                ...
                disabled={updating}
                className="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-dark-card text-red-600 dark:text-red-300 border border-red-200 dark:border-red-800/40 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition text-sm font-semibold disabled:opacity-50"
              >
      ...
      {showRevisionModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-dark-card rounded-xl shadow-2xl max-w-lg w-full p-6 border border-gray-200 dark:border-dark-border">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">Revision Notes</h3>
              <button onClick={() => setShowRevisionModal(false)} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300">
                <X size={20} />
              </button>
            </div>
            <p className="text-sm text-gray-500 dark:text-gray-400 font-light mb-3">Describe what needs to be changed:</p>
            <textarea
              value={revisionNotes}
              onChange={(e) => setRevisionNotes(e.target.value)}
              rows={4}
              className="w-full px-4 py-3 border !border-gray-300 dark:!border-dark-border !bg-white dark:!bg-dark-bg text-gray-900 dark:text-gray-200 rounded-lg focus:outline-none focus:border-primary text-sm font-light"
              placeholder="e.g., Dosing table needs updated values for ropivacaine..."
            />
            <div className="flex gap-3 mt-4 justify-end">
              <button
                onClick={() => setShowRevisionModal(false)}
                className="px-4 py-2 text-sm font-semibold text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 transition"
              >
                Cancel
              </button>
              <button
      ...
      <div className="bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-xl overflow-hidden">
        ...
        <div className="border-t border-gray-200 dark:border-dark-border px-6 py-4 bg-gray-50 dark:bg-dark-bg flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400 dark:text-gray-500 font-light">Foundation Package — Safety</span>
        </div>
      </div>
    </div>
  );
