...
import { ChevronDown, ChevronRight, FileText, RefreshCw, Loader2, Scale } from 'lucide-react';
...
export const LegalPage = () => {
  ...
  useEffect(() => {
    if (location.state?.reset) setOpenDoc(null);
  }, [location.state]);
  ...
  const fetchDocs = async () => {
    ...
    try {
      ...
      const docsWithMeta = await Promise.all(
        lists.flat().map(async (file) => {
          const { data: urlData } = supabase.storage.from('deliverables').getPublicUrl(file.path);
          try {
      ...
      setExpandedCategories(Object.fromEntries(sorted.map(c => [c.name, true])));
    } catch (err) {
