...
const requiredMarkers = new Map([
  ['KPI Dictionary and Data Ownership', 'M1 mock value-story metric set — approved 2026-07-21'],
  ['Measurement Framework and Minimum Dataset', 'approved M1 value story uses six separately governed placeholders'],
  ['CRM Pipeline Stages', 'M1 mock value-story set — approved 2026-07-21'],
]);
const lenses = [
...
const { data: beforeTask, error: taskError } = await client
  ...
  .select('*')
  .eq('task_key', 'M1-GR-025')
  .single();
fail('Read M1-GR-025', taskError);

const backupPath = `/tmp/blockops-before-m1-gr-025-${Date.now()}.json`;
...
if (!verifyOnly) {
  ...
  const approvalNote = 'Completed 2026-07-21: Samir approved six M1 mock value-story lenses—block utilization/opportunity, opioid-use impact, recovery/throughput impact, patient experience/quality, documentation/economic contribution, and net value/ROI. These are placeholders, not measured results or external claims. Unknown stays Unknown; mock assumptions remain labeled. Max owns source mapping, calculation implementation, and Dashboard representation; Bloq may enter labeled mock placeholders. Clinical/evidence, billing/compliance, legal, financial, and exact-wording reviews remain applicable. Definition approval does not prove implementation, capture, or ROI.';
  ...
  const { error } = await client.from('launch_tasks_v2').update({
    ...
    description: 'Define six approved value-story placeholders for the clearly labeled M1 mock client without creating measured results, external claims, or implementation proof.',
    notes,
  }).eq('id', beforeTask.id);
  fail('Complete M1-GR-025', error);
}
...
const { data: afterTask, error: afterTaskError } = await client
  ...
  .eq('task_key', 'M1-GR-025')
  .single();
fail('Verify M1-GR-025', afterTaskError);
...
const pages = titles.map((title) => {
  ...
  return {
    ...
    sixLensesPresent: lenses.every((marker) => body.includes(marker)),
...
const report = {
  ...
  task: {
    ...
    approvalNotePresent: String(afterTask.notes || '').includes('Samir approved six M1 mock value-story lenses'),
  },
  pages,
};
...
const failed = afterTask.status !== 'done' || !afterTask.completed_at || !report.task.approvalNotePresent
  || pages.some((page) => !page.markerPresent || !page.sixLensesPresent || !page.statusPreserved || !page.ownerPreserved || page.crossLinkBlocks > 1);
