...
import { MR_TRAINING_TASKS, MR_TRAINING_DEPENDENCIES, MR_TRAINING_COLLABORATORS } from '../src/services/mrTrainingProjectSeed.js';
...
function report(all){
 const keys=new Set(MR_TRAINING_TASKS.map(x=>x.taskKey));
 const rows=all.tasks.filter(x=>keys.has(x.task_key));
 const ids=new Set(rows.map(x=>x.id));
 const wf=rows.reduce((a,r)=>(a[r.workstream]=(a[r.workstream]||0)+1,a),{});
 const expectedWf=MR_TRAINING_TASKS.reduce((a,r)=>(a[r.workstream]=(a[r.workstream]||0)+1,a),{});
 const byMilestone=Object.fromEntries(all.milestones.map(m=>[m.slug,rows.filter(r=>r.milestone_id===m.id).length]));
 const out={expected:{tasks:MR_TRAINING_TASKS.length,dependencies:MR_TRAINING_DEPENDENCIES.length,collaborators:MR_TRAINING_COLLABORATORS.length,workflows:expectedWf},live:{projectTasks:rows.length,dependencies:all.dependencies.filter(x=>ids.has(x.task_id)).length,collaborators:all.collaborators.filter(x=>ids.has(x.task_id)).length,statuses:statusSummary(rows),byMilestone,workflows:wf,missingMilestone:rows.filter(x=>!x.milestone_id).length,thisWeek:rows.filter(x=>x.status==='this_week').length},matches:{allTasks:rows.length===MR_TRAINING_TASKS.length,allDependencies:all.dependencies.filter(x=>ids.has(x.task_id)).length===MR_TRAINING_DEPENDENCIES.length,allCollaborators:all.collaborators.filter(x=>ids.has(x.task_id)).length===MR_TRAINING_COLLABORATORS.length,workflowCounts:JSON.stringify(wf)===JSON.stringify(expectedWf),noMissingMilestone:rows.every(x=>x.milestone_id),noThisWeek:rows.every(x=>x.status!=='this_week')}};
...
if(MR_TRAINING_DEPENDENCIES.length){r=await c.from('launch_task_dependencies').insert(MR_TRAINING_DEPENDENCIES.map(x=>({task_id:id.get(x.taskKey),depends_on_task_id:id.get(x.dependsOnTaskKey),dependency_type:x.dependencyType})));fail('insert project dependencies',r.error)}
if(MR_TRAINING_COLLABORATORS.length){r=await c.from('launch_task_collaborators').insert(MR_TRAINING_COLLABORATORS.map(x=>({task_id:id.get(x.taskKey),collaborator:x.collaborator})));fail('insert project collaborators',r.error)}
report(await read());
